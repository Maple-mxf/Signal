...
final class DistributeSemaphoreImp extends DistributeMongoSignalBase
  ...
  @VisibleForTesting volatile int occupiedPermits;

  @VisibleForTesting final AtomicLong occupiedPermitsRevision;
  ...
  DistributeSemaphoreImp(
    ...
    this.occupiedPermitsRevision = new AtomicLong(0L);
  }
  ...
  public void acquire(int permits, Long waitTime, TimeUnit timeUnit) throws InterruptedException {
    ...
    BiFunction<ClientSession, MongoCollection<Document>, TxnResponse> command =
        (session, collection) -> {
          long currOccupiedPermitsRevision = occupiedPermitsRevision.get();
          ...
          if (available < permits) {
            if (occupiedPermitsRevision.compareAndSet(
                currOccupiedPermitsRevision, currOccupiedPermitsRevision + 1)) {
              this.occupiedPermits = occupied;
              return parkThread();
            }
    ...
    for (; ; ) {
      boolean elapsed =
          parkCurrentThreadUntil(
              ...
              () -> permits > (this.permits() - occupiedPermits),
              timed,
              s,
  ...
  public void release(int permits) {
    ...
    if (!rsp.txnOk) throw new SignalException(rsp.message);
  }
  ...
  void awakeHead(ChangeStreamEvents.SemaphoreChangeAndRemovedEvent event) {
    ...
    for (; ; ) {
      ...
      if (occupiedPermitsRevision.compareAndSet(
          currOccupiedPermitsRevision, currOccupiedPermitsRevision + 1)) {
        this.occupiedPermits = occupiedPermits;
        this.doUnparkSuccessor();
        return;
      }
      Thread.onSpinWait();
