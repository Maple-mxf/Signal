...
public class DistributeSemaphoreImp extends DistributeMongoSignalBase
  ...
  @VisibleForTesting volatile StateVars<Integer> stateVars;
  ...
  DistributeSemaphoreImp(
    ...
    this.stateVars = new StateVars<>(0, 0L);
    try {
      varHandle =
  ...
  public void acquire(int permits, Long waitTime, TimeUnit timeUnit) throws InterruptedException {
    ...
    BiFunction<ClientSession, MongoCollection<Document>, TxnResponse> command =
        (session, collection) -> {
          StateVars<Integer> currState = this.stateVars;
          int currStateAddr = System.identityHashCode(currState);
          ...
          if (available < permits) {
            if (System.identityHashCode(
                    varHandle.compareAndExchangeRelease(
                        this, currState, new StateVars<>(occupied, currState.revision)))
                == currStateAddr) {
              return parkThread();
            }
  ...
  void awakeHead(ChangeStreamEvents.SemaphoreChangeAndRemovedEvent event) {
    ...
    for (; ; ) {
      ...
      if (event.fullDocument() == null) {
        if (System.identityHashCode(
                varHandle.compareAndExchangeRelease(
                    this, currState, new StateVars<>(0, currState.revision)))
            == currStateAddr) {
      ...
      if (System.identityHashCode(
              varHandle.compareAndExchangeRelease(
                  this, currState, new StateVars<>(occupiedPermits, currState.revision)))
          == currStateAddr) {
        this.doUnparkSuccessor();
        return;
      }
