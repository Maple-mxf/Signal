...
public final class DistributeMutexLockImp extends DistributeMongoSignalBase
  ...
  @VisibleForTesting volatile boolean occupied;
  @VisibleForTesting final AtomicLong occupiedRevision;
  ...
  public DistributeMutexLockImp(
    ...
    this.occupied = false;
    this.occupiedRevision = new AtomicLong(0L);
  ...
  public boolean tryLock(Long waitTime, TimeUnit timeUnit) throws InterruptedException {
    ...
    BiFunction<ClientSession, MongoCollection<Document>, TxnResponse> command =
        (session, coll) -> {
          long currOccupiedRevision = occupiedRevision.get();
          ...
          if (optional.isEmpty() && !holdersEmpty) {
            if (occupiedRevision.compareAndSet(currOccupiedRevision, currOccupiedRevision + 1)) {
              occupied = true;
              return parkThread();
            }
    ...
    for (; ; ) {
      // 尝试着挂起当前线程
      boolean elapsed =
          parkCurrentThreadUntil(lock, available, () -> occupied, timed, s, waitTimeNanos);
