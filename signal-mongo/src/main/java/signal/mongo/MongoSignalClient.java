...
public class MongoSignalClient implements SignalClient {
  ...
  private final Table<String, OperationType, Consumer<ChangeStreamDocument<Document>>>
      changeStreamEventsDispatcher;
  ...
  private MongoSignalClient(MongoClient mongoClient, String dbNamed) {
    ...
    this.changeStreamEventsDispatcher = HashBasedTable.create();
    this.leaseWrapperList = new ArrayList<>(8);
    this.leaseWrapperRWLock = new ReentrantReadWriteLock(true);
    this.setupChangeStreamEventDispatcher();
  ...
  private void setupChangeStreamEventDispatcher() {
    ...
    changeStreamEventsDispatcher.put(
        LEASE_NAMED,
        DELETE,
        streamDocument -> {
        ...
        (scopedEvent, leaseIdList) -> {
          ...
          try {
            leaseWrapperList.stream()
                ...
                .forEach(
                    w -> {
                      w.lease.postScopedEvent(scopedEvent);
                    });
          } finally {
    ...
    BiConsumer<ChangeStreamDocument<Document>, Function<ChangeStreamDocument<Document>, Object>>
        eventHandlerFn =
            (streamDocument, eventProducer) -> {
              Collection<String> leaseIdList = new LinkedList<>();
    ...
    changeStreamEventsDispatcher.put(
        ...
        DELETE,
        streamDocument -> {
          String barrierKey = getKeyFn.apply(streamDocument);
          eventHandlerFn.accept(
              streamDocument,
              t ->
                  new ChangeStreamEvents.BarrierRemovedEvent(
                      barrierKey, streamDocument.getFullDocumentBeforeChange()));
        });
            ...
            eventHandlerFn.accept(
                ...
                t ->
                    new ChangeStreamEvents.SemaphoreChangeAndRemovedEvent(
                        getKeyFn.apply(streamDocument),
                        streamDocument.getFullDocument().getInteger("p"),
    ...
    changeStreamEventsDispatcher.put(
        ...
        streamDocument ->
            eventHandlerFn.accept(
                ...
                t ->
                    new ChangeStreamEvents.ReadWriteLockChangeAndRemovedEvent(
                        t.getDocumentKey().getString("_id").getValue(), null)));
  }
  ...
  private class ChangeStreamWatcher implements Runnable {
    ...
    public void run() {
      ...
      for (ChangeStreamDocument<Document> streamDocument : changeStream) {
        ...
        try {
          Consumer<ChangeStreamDocument<Document>> function =
              changeStreamEventsDispatcher.get(
                  namespace.getCollectionName(), streamDocument.getOperationType());
          if (function == null) continue;
          function.accept(streamDocument);
  ...
  private class CleanExpireSignalTask extends TimerTask {
    ...
    public void run() {
      ...
      try {
        var expiredLeaseCollNamedBinds = new LinkedList<ExpiredSignalCollNamedBind>();
        ...
        expireLeaseGroup.forEach(
            (collectionNamed, subExpiredLeaseCollNamedBinds) -> {
              ...
              if (MUTEX_LOCK_NAMED.equals(collectionNamed)) {
                ...
                var writeOps = new LinkedList<UpdateOneModel<Document>>();
  ...
  Collection<ExpiredSignalCollNamedBind> findExpireExclusiveSignal(String collectionNamed) {
    var pipeline = new LinkedList<Bson>();
