...
abstract class DistributeMongoSignalBase extends DistributeSignalBase {
  ...
  static Optional<Document> extractHolder(Document sharedSignal, Document holder) {
    List<Document> holders = sharedSignal.getList("o", Document.class);
  ...
  protected Collection<Holder> doGetHolders() {
    ...
    return commandExecutor.loopExecute(
        command,
        commandExecutor.defaultDBErrorHandlePolicy(ImmutableSet.of(NoSuchTransaction)),
        null,
        t -> false);
  }
