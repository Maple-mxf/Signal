...
public final class DistributeReadWriteLockImp extends DistributeMongoSignalBase
   ...
   * </ul>
   */
  @VisibleForTesting volatile String state;

  /** 用于控制{@link DistributeReadWriteLockImp#state}的版本，每更新一次state的值，必须要并发临界区内将stateRevision的值加1 */
  @VisibleForTesting final AtomicLong stateRevision = new AtomicLong(0L);
    ...
    public boolean tryLock(Long waitTime, TimeUnit timeUnit) throws InterruptedException {
      ...
      BiFunction<ClientSession, MongoCollection<Document>, TxnResponse> command =
          (session, coll) -> {
            ...
            long currStateRevision = stateRevision.get();
            ...
            if (inReadMode && !holdersEmpty) {
              // 设置当前锁的状态处于r状态
              if (stateRevision.compareAndSet(currStateRevision, currStateRevision + 1)) {
                state = "r";
                return parkThread();
              }
      ...
      for (; ; ) {
        ...
        boolean elapsed =
            parkCurrentThreadUntil(
                lock,
                available,
                () -> !(state == null || state.isEmpty() || "r".equals(state)),
                timed,
                s,
                waitTimeNanos);
  ...
  /**
   * 满足条件会唤醒头部线程 并且将{@link DistributeWriteLockImp#state} 设置为空或者"r"
   *
  ...
  void awakeHead(ChangeStreamEvents.ReadWriteLockChangeAndRemovedEvent event) {
    ...
    Next:
    for (; ; ) {
      // 执行之前先获取state revision的值，避免和lock函数并发更新state的值冲突
      long currStateRevision = stateRevision.get();
